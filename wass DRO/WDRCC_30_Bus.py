...
def DRO_30_bus(model,zeta,epsilon,p_w_f):
    units = pd.read_excel('data\IEEE 30 bus.xlsx', 'Generator')     # Parameters of generators
    branch = pd.read_excel('data\IEEE 30 bus.xlsx', 'Branch')       # Parameters of branch
    Load = pd.read_excel('data\IEEE 30 bus.xlsx', 'Load')           # Paremeters of Load
    ...
    p_ch_max = np.array([20,30,60])           # 充电最大值
    p_dis_max = np.array([20,30,60])          # 放电最大值
    S_ess_max = np.array([200,250,500])        # 储能最大值 
    ...
    # np.random.seed(0)
    # p_w_f_max = 150
    # p_w_f = np.random.normal(110, np.sqrt(1000), (T,W))
    ...
    wind_nodes = np.array([5,7])

    ess_nodes = np.array([4,6,7])                   # 储能节点
    ess_nodes = np.array([4])
    ...
    if flag:
        ...
        # 显示图表
        plt.show()
    return u.get(),v.get(),w.get(),g.get(),p_w_cut.get(),sum_power_GSDF.get(),u_ch.get(),u_dis.get(),p_ch.get(),p_dis.get(),S_ess.get(),lambda_.get(),alpha_g.get(),alpha_w.get(),alpha_dis.get(),alpha_ch.get(),up_sr.get(),down_sr.get(),g_alpha_g.get(),Q,model.get(),t1.get(),t2.get(),solution_times,ops
...
if __name__ == '__main__':
    ...
    winddata = (wind_f-wind_t)/wind_capacity*200
    # random.seed(0)
    ...
    for num in nums:
        for epoch in range(random_times):
            ...
            model = ro.Model('DRO')         
            u,v,w,g,p_w_cut,sum_power_GSDF,u_ch,u_dis,p_ch,p_dis,S_ess,lambda_,alpha_g,alpha_w,alpha_dis,alpha_ch,up_sr,down_sr,g_alpha_g,Q,op_ob,t1,t2,solution_time,ops = DRO_30_bus(model,zeta,epsilon)
    ...
    os.makedirs('exp_data\IEEE_30_bus_RO_SO_DRO', exist_ok=True)
    np.save('exp_data\IEEE_30_bus_RO_SO_DRO\WDRCC.npy',op_dro)
    np.save('exp_data\IEEE_30_bus_RO_SO_DRO\RO.npy',op_ro)
    np.save('exp_data\IEEE_30_bus_RO_SO_DRO\SO.npy',op_so)
    ...
    plt.bar(x + bar_width, op_dro.mean(axis=0), width=bar_width, color='#ff847b', label='WDRCC', align='center')
    plt.bar(x + 2 * bar_width, op_so.mean(axis=0), width=bar_width, color='#a2d2bf', label='SO', align='center')

    # 添加数据标签
    for i in range(len(nums)):
        plt.text(i, op_ro.mean(axis=0)[i] + 2000, f"{int(op_ro.mean(axis=0)[i])}", ha='center', color='black')
        plt.text(i + bar_width, op_dro.mean(axis=0)[i] + 2000, f"{int(op_dro.mean(axis=0)[i])}", ha='center', color='black')
        plt.text(i + 2 * bar_width, op_so.mean(axis=0)[i] + 2000, f"{int(op_so.mean(axis=0)[i])}", ha='center', color='black')

    # 设置x轴标签和图例
    plt.xticks(x + bar_width, nums)
    plt.legend(loc='upper left')
