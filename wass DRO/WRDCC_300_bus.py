...
def WDRCC_300_bus(model,zeta,epsilon,p_w_f):
    ## 1.读取数据
    units = pd.read_excel('data\IEEE 300 bus.xlsx', 'Generator')     # Parameters of generators
    branch = pd.read_excel('data\IEEE 300 bus.xlsx', 'Branch')       # Parameters of branch
    Load = pd.read_excel('data\IEEE 300 bus.xlsx', 'Load')           # Paremeters of Load
    ...
    load = Load['load_all'].values[0:24]+10000
    ...
    p_ch_max = np.array([120,180,360,120,180,360])           # 充电最大值
    p_dis_max = np.array([120,180,360,120,180,360])          # 放电最大值
    S_ess_max = np.array([800,1000,2000,800,1000,2000])        # 储能最大值 

    p_ch_max = p_ch_max[:ESS]
    p_dis_max = p_dis_max[:ESS]
    S_ess_max = S_ess_max[:ESS]

    eta_ch = np.array([0.9,0.9,0.9,0.9,0.9,0.9])
    eta_dis = np.array([0.9,0.9,0.9,0.9,0.9,0.9])
    ...
    model.st(S_ess_initial == S_ess[T],S_ess[0] == S_ess_initial, S_ess[1:] == S_ess[:-1] + eta_ch*p_ch[:]-1/eta_dis*p_dis[:])   # 储能前后状态约束
    ...
    for k in range(lines):
        # i = from_node[k]-1
        # j = to_node[k]-1
        i = np.where(load_bus == from_node[k])[0]
        j = np.where(load_bus == to_node[k])[0]
    ...
    wind_nodes = np.array([3,5,7,16,21,23,80,112,143,172,214,243,323,9002])         # 风电节点
    wind_nodes = wind_nodes[0:W]
    ess_nodes = np.array([4,25,51,91,123,155])                   # 储能节点
    ess_nodes = ess_nodes[0:ESS]
    ...
    load_rate_new[np.where(load_bus == load_bus)[0]] = load_rate           # 获取每个负荷节点的注入率
    G_generator = G[:,np.where(generator_nodes == generator_nodes)[0]]            # 获取有负荷注入的PTDF矩阵
    G_wind = G[:,np.where(wind_nodes == wind_nodes)[0]]                      # 获取风电的PTDF矩阵
    G_ess = G[:,np.where(ess_nodes == ess_nodes)[0]]                        # 获取储能的PTDF矩阵
    # 获取负荷注入以及所有机组出力注入
    for t in range(T):
    ...
    ## 7 模型求解
    # model.solve(grb, params={'MIPGap': 1e-1},log='Ture')                                    # Solve the model
    model.solve(cpt,params={'MIPGap': 1e-1},display='True')
    ...
    if flag:
        ...
        plt.bar(times, cutting_wind, bottom=thermal_power + wind_power, color='#a2d2bf', label='Cutting wind')
        ...
        # 显示图表
        plt.show()
    return model.get(),solution_times,ops, p_w_cut.get(),up_sr.get(),down_sr.get()
...
if __name__ == '__main__':
    ...
    winddata = (wind_f-wind_t)/wind_capacity*2000

    np.random.seed(0)
    all_list = range(winddata.shape[0])

    p_w_f_max = 2000
    p_w_f = np.random.normal(1200, np.sqrt(50000), (24,12))
    ...
    for num in nums:
        for epoch in range(random_times):
            ...
            model = ro.Model('DRO')         
            op_ob,solution_times,ops,p_w_cut,up_sr,down_sr = WDRCC_300_bus(model,zeta,epsilon,p_w_f)
